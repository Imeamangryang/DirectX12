...
namespace graphics {
	...
	{		
		...
		// 2. Device 생성
		{
			...
			while (m_dxgiFactory->EnumAdapters1(adapterIndex, &m_adapter) != DXGI_ERROR_NOT_FOUND)
			{
				...
				if (SUCCEEDED(D3D12CreateDevice(m_adapter, FEATURE_LEVEL, _uuidof(ID3D12Device), NULL))) {
					adapterFound = true;
					break;
				}
			...
			if (FAILED(D3D12CreateDevice(m_adapter, FEATURE_LEVEL, IID_PPV_ARGS(&m_device)))) 
			{
		...
		// 4. Swap chain 생성
		{
			...
			if (FAILED(m_dxgiFactory->CreateSwapChain(m_commandQueue, &swapChainDesc, &swapChain))) 
			{
		...
		// 8. CommandList 생성
		{
			if (FAILED(m_device->CreateCommandList(0, D3D12_COMMAND_LIST_TYPE_DIRECT, m_commandAllocator[m_BufferIndex], NULL, IID_PPV_ARGS(&m_commandList)))) 
			{
	...
	{
		if (FAILED(m_commandQueue->Signal(m_fence[m_BufferIndex], m_fenceValues[m_BufferIndex])))
		{
	...
	{
		ID3D12CommandList* lCmds[] = { m_commandList };
	...
	void Graphics::ResetPipeline()
	{
		...
		if (FAILED(m_commandList->Reset(m_commandAllocator[m_BufferIndex], NULL)))
		{
	...
	void Graphics::SetBackBufferRender(ID3D12GraphicsCommandList* commnadList, const float clearColor[4])
	{
	...
	void Graphics::SetBackBufferPresent(ID3D12GraphicsCommandList* commnadList)
	{
		commnadList->ResourceBarrier(
			1,
			&CD3DX12_RESOURCE_BARRIER::Transition(
...
;	}
	...
	void Graphics::ClearAllFrames()
	{
		for (int i = 0; i < FRAME_BUFFER_COUNT; ++i)
		{
			...
			if (FAILED(m_commandQueue->Signal(m_fence[i], m_fenceValues[i]))) 
			{
				throw GFX_Exception("CommandQueue Signal Fence failed on Render.");
			}
